...
volatile bool lastButtonState = LOW;   // Last sampled state of the button
volatile bool debouncedState = LOW;    // Debounced state of the button
volatile bool currentStatus = LOW;     // Current stable state after debouncing
volatile bool lastStableState = LOW;   // Last stable state
...
void setup() {
    ...
    set_PE_as_input(4);
    *ddr_a |= (1 << PA2);  // Set PA2 as output

    // Timer setup for button sampling every 1ms
    noInterrupts();  // Disable interrupts for setting changes
    *myTCCR1A = 0;   // Clear Timer1 settings
    *myTCCR1B = 0;
    *myTCNT1  = 0;
    *myOCR1A  = 16000 - 1;  // Set compare match register for 1ms at 16 MHz
    *myTCCR1B |= (1 << WGM12);  // CTC mode
    *myTCCR1B |= (1 << CS10);   // No prescaler
    *myTIMSK1 |= (1 << OCIE1A); // Enable timer compare interrupt
    interrupts();  // Enable interrupts
}

ISR(TIMER1_COMPA_vect) {
    bool readState = readButtonPE4();  // Use the custom function to read the button state

    if (readState == lastButtonState) { // Check if the state is stable
        if (readState != debouncedState) { // Update the debounced state if it is stable and has changed
            debouncedState = readState;
            currentStatus = debouncedState; // Update currentStatus to reflect the new stable state
        }
    }
    lastButtonState = readState; // Update lastButtonState for the next comparison
}

void loop() {
    // Check for a state change and act only on change
    if (currentStatus != lastStableState) {
        setLEDPA2(currentStatus);  // Use the custom function to set the LED state
        lastStableState = currentStatus; // Update the last stable state
    }
...
bool readButtonPE4() {
    return (*pin_e & (1 << PE4)) == 0;
}

void setLEDPA2(bool state) {
    if (state) {
        *port_a |= (1 << PA2);
    } else {
        *port_a &= ~(1 << PA2);
